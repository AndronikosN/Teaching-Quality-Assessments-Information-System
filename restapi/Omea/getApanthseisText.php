...
$query = "SELECT apanthseis.ans FROM apanthseis WHERE quest_id=$quest_id AND mathhma_id=$class_id";
$stmt = $db->prepare($query);

$stmt->execute();
$num = $stmt->rowCount();

$apanthseisText_array = array();
$apanthseisText_array["records"] = array();
...
if($num>0) {
    ...
    while($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
        ...
        array_push($apanthseisText_array["records"], $apanthseis_item);
    ...
    echo json_encode($apanthseisText_array);
